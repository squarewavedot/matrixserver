...
CubeApplication::CubeApplication(int fps, std::string setServerAddress, std::string setServerPort) :
        ...
        pixels3D(boost::extents[VIRTUALCUBESIZE][VIRTUALCUBESIZE][VIRTUALCUBESIZE])
{
    clear();
    srand(time(NULL));
}
...
void CubeApplication::fillAll(Color col){
    std::fill(pixels3D.data(), pixels3D.data() + pixels3D.num_elements(), col);
}
...
void CubeApplication::render(){
    int sx, sy;
    for(sx = 0; sx < CUBESIZE; sx++){
        for(sy = 0; sy < CUBESIZE; sy++){
            screens[0]->setPixel(sx,sy,getPixel3D(sx+1,0,sy+1)); //Screen 0: 3d_y = 0; x = 3d_x; y = 3d_z;
            screens[1]->setPixel(sx,sy,getPixel3D(VIRTUALCUBEMAXINDEX,sx+1,sy+1)); //Screen 1: 3d_x = 63; 3d_y = x; 3d_z = y;
            screens[2]->setPixel(sx,sy,getPixel3D(VIRTUALCUBEMAXINDEX-sx-1,VIRTUALCUBEMAXINDEX,sy+1)); //Screen 2: 3d_x = 63-x; 3d_y = 63; 3d_z = y;
            screens[3]->setPixel(sx,sy,getPixel3D(0,VIRTUALCUBEMAXINDEX-sx-1,sy+1)); //Screen 3: 3d_x = 0; 3d_y = 63-x; 3d_z = y;
            screens[4]->setPixel(sx,sy,getPixel3D(sx+1,VIRTUALCUBEMAXINDEX-sy-1,0)); //Screen 4: 3d_x = x; 3d_y = 63-y; 3d_z = 0;
            screens[5]->setPixel(sx,sy,getPixel3D(sx+1,sy+1,VIRTUALCUBEMAXINDEX)); //Screen 5: 3d_x = x; 3d_y = y; 3d_z = 63;
        }
    }
}
